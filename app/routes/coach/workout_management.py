...
def create_workout():
    ...
    try:
        # Get athlete_id and verify access
        ...
        workout = WorkoutLog(
            ...
            workout_type=final_workout_type, # ✅ Use final_workout_type
            session_type='workout',
            planned_duration=request.form.get('planned_duration', type=int),
            actual_duration=request.form.get('planned_duration', type=int), # Use planned duration for now
...
def update_workout(workout_id):
    ...
    try:
        workout = db.session.query(WorkoutLog).join(CoachAthlete, CoachAthlete.athlete_id == WorkoutLog.athlete_id).filter(
            WorkoutLog.id == workout_id, CoachAthlete.coach_id == identity, CoachAthlete.is_active == True
        ).first()
        if not workout:
            return jsonify({"msg": "Workout not found"}), 404
        ...
        if 'title' in request.form: workout.title = request.form['title']
        if 'workout_type' in request.form: workout.workout_type = final_workout_type # ✅ Use final_workout_type
        if 'planned_duration' in request.form: workout.planned_duration = int(request.form['planned_duration'])
        if 'actual_duration' in request.form: workout.actual_duration = int(request.form['actual_duration'])
        if 'calories_burned' in request.form: workout.calories_burned = int(request.form['calories_burned'])
        if 'avg_heart_rate' in request.form: workout.avg_heart_rate = int(request.form['avg_heart_rate']) if request.form['avg_heart_rate'] else None
        if 'max_heart_rate' in request.form: workout.max_heart_rate = int(request.form['max_heart_rate']) if request.form['max_heart_rate'] else None
        if 'completion_status' in request.form: workout.completion_status = request.form['completion_status']
        if 'difficulty_level' in request.form: workout.difficulty_level = request.form['difficulty_level']
        if 'notes' in request.form: workout.notes = request.form['notes']
        if 'date' in request.form: workout.date = datetime.strptime(request.form['date'], '%Y-%m-%d').date()
        if 'recovery_time' in request.form: workout.recovery_time = int(request.form['recovery_time']) if request.form['recovery_time'] else None
